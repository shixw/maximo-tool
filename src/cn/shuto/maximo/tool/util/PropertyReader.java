...
public class PropertyReader {
	...
	private static Logger _log = Logger.getLogger(PropertyReader.class.getName());
	...
	public final static Properties getProperties(String propertyFilePath) {
		if (propertyFilePath == null) {
			_log.info("propertyFilePath is null!");
			return null;
		}
	...
	private static Properties loadPropertyFile(String propertyFilePath) {
		...
		try {
			...
			_log.info("加载属性文件出错:" + propertyFilePath);
			return null;
		}
	}
	...
	private static Properties loadPropertyFileByFileSystem(final String propertyFilePath) {
		try {
			...
			_log.info("FileInputStream(\"" + propertyFilePath + "\")! FileNotFoundException: " + e);
			return null;
		} catch (java.io.IOException e) {
			_log.info("Properties.load(InputStream)! IOException: " + e);
			return null;
		}
	}
	...
	public final static boolean setValueAndStore(String propertyFilePath,
		...
		try {
			...
			_log.info("propertyFilePath = " + propertyFilePath);
			String path = PropertyReader.class.getResource(propertyFilePath).getPath();
			_log.info("~~~~~~~~path~~~XXX~~~~~" + path);
			try {
				stream = new java.io.FileOutputStream(path);
			} catch (FileNotFoundException e1) {
				_log.info("FileNotFoundException! path=" + propertyFilePath);
				return false;
			}
		}
	...
	public final static void store(Properties properties, String propertyFilePath, String msg) {
		try {
			...
			_log.info("FileOutputStream(" + propertyFilePath + ")! FileNotFoundException: " + e);
		} catch (java.io.IOException e) {
			_log.info("store(stream, msg)! IOException: " + e);
			e.printStackTrace();
		}
	}
	...
	public final static boolean updateValue(String propertyFilePath, String key, String newValue) {
		if (key == null || newValue == null) {
			_log.info("key or newValue is null!");
			return false;
		}
